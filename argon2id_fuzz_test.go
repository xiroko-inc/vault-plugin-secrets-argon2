...
//
// We do NOT wrap the calls in defer/recover. `go test -fuzz`
// already treats a panic as a crash, saves the minimized
// reproducer to testdata/fuzz/, and emits the full stack trace.
// A recover wrapper would replace that stack trace with a
// `t.Fatalf` line and add per-iteration overhead.
...
func FuzzParsePHC(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, phc string) {
		...
		// never panic." A panic propagates up to Go's fuzz runner,
		// which fails the test, saves the minimized reproducer,
		// and emits the full stack trace.
